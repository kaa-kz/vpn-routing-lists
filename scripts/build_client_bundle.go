...
func readUniqueLines(path string) []string {
	...
	for _, raw := range strings.Split(string(b), "\n") {
		...
		if seen[line] {
			failf("duplicate in %s: %s", path, line)
		}
		seen[line] = true
		out = append(out, line)
	}
...
func loadServerBlock(dir string) ([]string, []string, ServerManifest) {
	...
	if err != nil {
		failf("read server manifest: %v", err)
	}
	var m ServerManifest
	if err := json.Unmarshal(mb, &m); err != nil {
		failf("decode server manifest: %v", err)
	}
	if m.Name != "SERVER_BLOCK" || m.Version == "" {
		failf("invalid server manifest")
	}

	parts := append([]string(nil), m.DomainParts...)
	if len(parts) == 0 {
		failf("server manifest has no domain parts")
	}
	sort.Strings(parts)
	domains := []string{}
	seen := map[string]bool{}
	...
	for _, name := range parts {
		...
		b, err := os.ReadFile(p)
		if err != nil {
			failf("read server domain part %s: %v", p, err)
		}
		...
		for _, raw := range strings.Split(string(b), "\n") {
			...
			if len(fields) != 2 || fields[0] != "domain-suffix" {
				failf("bad server domain line: %q", line)
			}
			v := fields[1]
			if seen[v] {
				failf("duplicate server domain: %s", v)
			}
			seen[v] = true
			domains = append(domains, v)
		}
	}
	if len(domains) != m.DomainSuffixCount {
		failf("server domains count mismatch: %d != %d", len(domains), m.DomainSuffixCount)
	}
	...
	for _, line := range ipv4Lines {
		fields := strings.Split(line, "\t")
		if len(fields) != 2 || fields[0] != "ip-cidr" {
			failf("bad server IPv4 line: %q", line)
		}
		ipv4 = append(ipv4, fields[1])
	}
	if len(ipv4) != m.IPv4CIDRCount {
		failf("server IPv4 count mismatch: %d != %d", len(ipv4), m.IPv4CIDRCount)
	}
	if len(domains)+len(ipv4) != m.TotalRules {
		failf("server total mismatch")
	}
...
func loadPolicy(path string) PolicyProfile {
	b, err := os.ReadFile(path)
	if err != nil {
		failf("read policy: %v", err)
	}
	var p PolicyProfile
	if err := json.Unmarshal(b, &p); err != nil {
		failf("decode policy: %v", err)
	}
	seen := map[string]bool{}
	for i := range p.Categories {
		...
		if p.Categories[i].Name == "" {
			failf("empty category name")
		}
		if p.Categories[i].Policy != "DIRECT" && p.Categories[i].Policy != "PROXY" {
			failf("unsupported policy %s", p.Categories[i].Policy)
		}
		if seen[p.Categories[i].Name] {
			failf("duplicate policy category %s", p.Categories[i].Name)
		}
...
func emitCategory(b *strings.Builder, site *router.GeoSite, cp CategoryPolicy, categoryCounts map[string]int, ruleCount *int) {
...
func main() {
	...
	for k, v := range map[string]*string{"runet-geosite": basePath, "ru-cleaned": ruPath, "server-block-dir": serverDir, "policy": policyPath, "happ-output": happOut, "shadowrocket-output": srOut, "manifest-output": manifestOut} {
		if *v == "" {
			failf("--%s is required", k)
		}
	}

	baseBytes, err := os.ReadFile(*basePath)
	if err != nil {
		failf("read Runet geosite: %v", err)
	}
	var base router.GeoSiteList
	if err := proto.Unmarshal(baseBytes, &base); err != nil {
		failf("decode Runet geosite: %v", err)
	}
	if len(base.Entry) == 0 {
		failf("Runet geosite has zero categories")
	}

	ru := readUniqueLines(*ruPath)
	if len(ru) == 0 {
		failf("ru-blocked-cleaned is empty")
	}
	...
	for _, site := range base.Entry {
		if custom[strings.ToUpper(site.CountryCode)] {
			continue
		}
	...
	for _, v := range ru {
		ruDomains = append(ruDomains, &router.Domain{Type: router.Domain_Full, Value: v})
	}
	sbDomains := make([]*router.Domain, 0, len(serverDomains))
	for _, v := range serverDomains {
		sbDomains = append(sbDomains, &router.Domain{Type: router.Domain_RootDomain, Value: v})
	}
	...
	if err != nil {
		failf("encode Happ geosite: %v", err)
	}
	if err := os.MkdirAll(filepath.Dir(*happOut), 0755); err != nil {
		failf("mkdir Happ output: %v", err)
	}
	if err := os.WriteFile(*happOut, happBytes, 0644); err != nil {
		failf("write Happ output: %v", err)
	}

	// Shadowrocket: all managed PROXY rules precede all managed DIRECT rules.
	var b strings.Builder
	ruleCount := 0
	b.WriteString("# Generated client routing rules. Rule order is significant.\n")
	b.WriteString("# All managed PROXY rules precede all managed DIRECT rules.\n")
	b.WriteString("[Rule]\n")
	b.WriteString("# ru-blocked-cleaned -> PROXY\n")
	for _, v := range ru {
		b.WriteString(fmt.Sprintf("DOMAIN,%s,PROXY\n", v))
		ruleCount++
	}

	siteMap := map[string]*router.GeoSite{}
	for _, site := range base.Entry {
		siteMap[strings.ToUpper(site.CountryCode)] = site
	}
	...
	for _, cp := range policy.Categories {
		...
		if site == nil {
			failf("Runet category not found: %s", cp.Name)
		}
		emitCategory(&b, site, cp, categoryCounts, &ruleCount)
	}

	b.WriteString("# SERVER_BLOCK -> DIRECT\n")
	for _, v := range serverDomains {
		b.WriteString(fmt.Sprintf("DOMAIN-SUFFIX,%s,DIRECT\n", v))
		ruleCount++
	}
	for _, v := range serverIPv4 {
		b.WriteString(fmt.Sprintf("IP-CIDR,%s,DIRECT,no-resolve\n", v))
		ruleCount++
	}
	...
	if err := os.MkdirAll(filepath.Dir(*srOut), 0755); err != nil {
		failf("mkdir Shadowrocket output: %v", err)
	}
	if err := os.WriteFile(*srOut, srBytes, 0644); err != nil {
		failf("write Shadowrocket output: %v", err)
	}
	...
	if err := os.MkdirAll(filepath.Dir(*manifestOut), 0755); err != nil {
		failf("mkdir manifest output: %v", err)
	}
	if err := os.WriteFile(*manifestOut, mbytes, 0644); err != nil {
		failf("write manifest: %v", err)
	}
